...
data_nmds <- decostand(data_nmds, "chi.square")

nmds_results <- metaMDS(data_nmds, distance = "bray", k = 2, trymax = 100)
...
ggsave("./../figs/01_benthic-composition_nmds.png", height = 4, width = 5) # PNG
...
data_nmds <- data_benthic %>% 
  ...
  group_by(island) %>% 
  column_to_rownames(var = "year") %>% # Put year as rowname for the NMDS
  do(metaMDS(data_nmds, distance = "bray", k = 2, trymax = 100))
...
data_recruits <- data_recruits %>% 
  ...
  select(site, year, taxid, recruit) %>% 
...
ggplot(data_recruits, aes(x = year, y = recruit, group = year)) +
  geom_boxplot() +
  facet_wrap(~taxid, scales = "free", ncol = 5)

```

## Total recruits by island
...
## Total recruits by island and species
