...
lm_result <- lm(recruit ~ cover*island, data = data_all)
...
ggplot(data = data_all, aes(x = cover, y = recruit)) +
  ...
  geom_abline(intercept = 4.2155, slope = 0.0651) +
  #geom_smooth(method = lm, color = "#4183d7") +
  labs(x = "Hard coral cover (%)", y = "Coral recruit (n.m-2)") +
  annotate(geom = "text", Inf, -Inf, 
           label = paste0("Adjusted r² = ", round(summary(lm_result)$r.squared, 3)), 
           color = "black",
           hjust = 1.1, vjust = -2,
           family = font_choose_graph)
...
data_sst_all <- read.csv("./../data/02_sst-extracted.csv") %>% 
  group_by(date) %>% 
  summarise(sst = mean(sst)) %>% 
  mutate(date = as.Date(date))
...
plot_hcc <- ggplot(data = data_benthic_mean, aes(x = date, y = cover)) +
  geom_point(color = "grey", alpha = 0.5) +
  geom_smooth() +
  # DHW events
  geom_vline(xintercept = as.Date("2010-06-12"), col = "red") +
